...
def example_verify_weight_shapes():
    ...
    if stats["skipped_layers"]:
        print(
            f"  跳过的层: {stats['skipped_layers'][:10]}..."
            if len(stats["skipped_layers"]) > 10
            else f"  跳过的层: {stats['skipped_layers']}"
        )

    print(f"\n已加载的权重（前10个）:")
    for name in stats["loaded_weight_names"][:10]:
        print(f"  - {name}")
    if len(stats["loaded_weight_names"]) > 10:
        print(f"  ... 还有 {len(stats['loaded_weight_names']) - 10} 个权重")

    if stats["skipped_weight_names"]:
        print(f"\n跳过的权重（前10个）:")
        for name in stats["skipped_weight_names"][:10]:
            print(f"  - {name}")
        if len(stats["skipped_weight_names"]) > 10:
            print(f"  ... 还有 {len(stats['skipped_weight_names']) - 10} 个权重")
