...
Qwen2ForCausalLM 使用 QKVParallelLinear 和 MergedColumnParallelLinear 实现权重打包，
...
def build_qwen2_weight_mappings(num_hidden_layers: int) -> dict[str, WeightMapping]:
    """构建 Qwen2 模型的权重映射规则.
    ...
    for layer_id in range(num_hidden_layers):
        ...
        # 如果 qkv_bias=True（Qwen2 默认 True），还需要映射 bias
        # 注意：Qwen2 的 qkv_bias 默认为 True
